...
import SpeakScreenOriginal from './index_original';
import { getFallbackApiData } from '@/constants/FallbackChatOptions';
...
export default function SpeakScreen() {
  ...
  // If feature flag is disabled, use the original version
  if (!headerLanguageSwitcher) {
    return <SpeakScreenOriginal />;
  }
  ...
  const { userSettings, loading: userSettingsLoading, fetchUserSettings, updateUserSettings } = React.useContext(UserSettingsContext)!;
  ...
  const { isJapaneseReadingAidLoading } = useReadingAid();
  useFocusEffect(
    React.useCallback(() => {
      fetchApiData();
    }, [])
  ...
  // Refetch API data when language changes (with delay to avoid race condition)
  useEffect(() => {
    if (userSettings?.team.stream_language) {
      // Add a short pause to ensure settings are properly updated before fetching new voice data
      const timeoutId = setTimeout(() => {
        fetchApiData();
      }, 300); // 300ms pause to avoid race condition with voice selection
      
      return () => clearTimeout(timeoutId);
    }
  }, [userSettings?.team.stream_language]);

  useEffect(() => {
    if (userSettings && apiData) {
      const userLevel = userSettings?.team.stream_language_level || 'beginner';
      const chatSettings = userSettings.team.chat_settings;
      const currentLanguage = userSettings.team.stream_language || 'english';
      
      // Get voice options for current language
      const languageVoices = apiData.chat_form.all_voice_options[currentLanguage] || 
                            apiData.chat_form.all_voice_options[apiData.chat_form.language] || 
                            apiData.chat_form.all_voice_options['default'] || {};
      
      // Trust the voice from API data first (it has the correct dialect-specific voice)
      // Fall back to chat settings voice, then to first available voice
      let selectedVoice = apiData?.chat_form.voice || chatSettings.voice || '';
      
      // Only validate if we don't have a voice at all
      if (!selectedVoice) {
        const voiceValues = Object.values(languageVoices);
        selectedVoice = voiceValues[0] as string || '';
      }
  ...
  const fetchApiData = async () => {
    ...
    try {
      ...
      try {
        await AsyncStorage.setItem('cached_chat_options', JSON.stringify(data));
      } catch (cacheError) {
        console.error('Error caching chat options:', cacheError);
      }
    } catch (error) {
      ...
      try {
        const cachedData = await AsyncStorage.getItem('cached_chat_options');
        if (cachedData && !apiData) {
          setApiData(JSON.parse(cachedData));
          setNetworkError(false); // Don't show error if we have cached data
        }
      } catch (cacheError) {
        console.error('Error loading cached data:', cacheError);
      }
      
      // Always use language-specific fallback data when offline
      if (!apiData) {
        const fallbackData = getFallbackApiData(userSettings?.team.stream_language);
        setApiData(fallbackData);
      } else {
        // Update voice options in existing data to match current language
        const currentLanguage = userSettings?.team.stream_language || 'english';
        const fallbackData = getFallbackApiData(currentLanguage);
        setApiData(prevData => ({
          ...prevData!,
          chat_form: {
            ...prevData!.chat_form,
            voice_options: fallbackData.chat_form.voice_options,
            language: currentLanguage,
            voice: Object.values(fallbackData.chat_form.voice_options)[0] as string
          }
        }));
      }
    } finally {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/v1/chats`, {
        ...
        body: JSON.stringify({
          ...
          speed: 'standard',
          language: userSettings?.team.stream_language || apiData?.chat_form.language || 'spanish',
  ...
  const getHumanReadableVoice = (voice: string): string => {
    const currentLanguage = userSettings?.team.stream_language || apiData?.chat_form.language || 'english';
    ...
    // Primary: Check the current voice_options from API
    if (apiData.chat_form.voice_options) {
      const voiceEntry = Object.entries(apiData.chat_form.voice_options).find(([_, value]) => value === voice);
      if (voiceEntry) {
        return voiceEntry[0];
      }
    }

    // First try current language, then fallback to apiData language, then default
    const voiceOptions = apiData.chat_form.all_voice_options[currentLanguage] || 
                        apiData.chat_form.all_voice_options[apiData.chat_form.language] ||
                        apiData.chat_form.all_voice_options['default'];
                        
    if (voiceOptions) {
      const voiceEntry = Object.entries(voiceOptions).find(([_, value]) => value === voice);
      if (voiceEntry) {
        return voiceEntry[0];
      }
    }
    
    // If not found in current language options, search through all language options
    if (apiData.chat_form.all_voice_options) {
      for (const language in apiData.chat_form.all_voice_options) {
        const languageVoiceOptions = apiData.chat_form.all_voice_options[language];
        const voiceEntry = Object.entries(languageVoiceOptions).find(([_, value]) => value === voice);
        if (voiceEntry) {
          return voiceEntry[0];
        }
      }
    ...
    // Final fallback: search through ELEVENLABS_CHAT_VOICE_OPTIONS
    for (const language in ELEVENLABS_CHAT_VOICE_OPTIONS) {
      for (const label in ELEVENLABS_CHAT_VOICE_OPTIONS[language]) {
        if (ELEVENLABS_CHAT_VOICE_OPTIONS[language][label] === voice) {
          return label;
  ...
  const Header = ({ showBackButton = false, onBackPress, hideInfoSection = false, isLoading = false }: {
    ...
    <View style={[styles.header, { backgroundColor: isDark ? Colors.dark.background : Colors.light.background }]}>
      ...
      {isLoading && !isJapaneseReadingAidLoading && (
        <View style={styles.chatInfoContainer}>
          <View style={styles.chatInfoContent}>
      ...
      {isLoading && isJapaneseReadingAidLoading && (
        <View style={styles.chatInfoContainer}>
          <View style={styles.chatInfoContent}>
            <Text style={GlobalFontStyleSheet.textBase} className='text-gray-950 dark:text-white mb-2 text-center'>
              Loading japanese reading aids and dictionary...
            </Text>
          </View>
        </View>
      )}
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDark ? Colors.dark.background : Colors.light.background }]}>
      ...
      {networkError && (
        <Notification
          message="Unable to load chat options. Please check your internet connection."
          type="error"
