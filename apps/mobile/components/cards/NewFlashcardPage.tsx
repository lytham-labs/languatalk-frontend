...
import PracticeModal from '@/components/cards/ExerciseModal';
...
export default function CardsScreen() {
  ...
  useFocusEffect(
    React.useCallback(() => {
      // Reset layout stability when the screen comes into focus
      setIsLayoutStable(false);
      
      // Give time for tab layout to stabilize before showing content
      const layoutTimer = setTimeout(() => {
        setIsLayoutStable(true);
      }, 100); // Small delay to ensure tabs are rendered first
  ...
  // Additional effect to handle layout changes when navigating back
  useEffect(() => {
    // Reset layout stability on mount
    setIsLayoutStable(false);
    
    const stabilizeTimer = setTimeout(() => {
      setIsLayoutStable(true);
    }, 150); // Slightly longer delay for initial mount
  ...
  const ActionButton = ({
    ...
    const handlePress = () => {
      ...
      if (title === "Chat" || title === "Choose Exercise") {
        onPress();
        return;
      }
    ...
    const buttonClasses = cx(
      ...
      {
        "bg-red-400": title !== "Chat" && title !== "Choose Exercise",
    ...
    // Render Practice button with gradient
    if (title === "Choose Exercise") {
      return (
        <TouchableOpacity activeOpacity={0.8} onPressOut={handlePress} >
          <LinearGradient
            colors={['#F87171', colorScheme.get() === "dark" ? '#db4d4d' : '#FCA5A5']}
            start={{x: 0, y: 0}}
            end={{x: 2, y: 0}}
            style={{
              position: 'absolute',
              ...
              width: screenSizeCategory === 'large' ? 270 : 190,
              height: 50,
              borderRadius: 100,
              paddingVertical: 15,
          ...
          >
            <View className="flex-row items-center justify-center gap-4">
              <View className="bg-[#FCA5A5] rounded-full p-2">
                <FontAwesomeIcon icon={faBrain} size={16} color="#fff" />
              </View>
              <ThemedText
              style={[
  ...
  const handleModeNavigation = (title: string) => {
    // Close modal first
    ...
    if (title === 'Chat') {
      ...
      setTimeout(() => {
        router.setParams({ selectedOption: 'Vocab & Games' } as any);
      }, 100);
      return;
    }
    ...
    if (Object.keys(activeFilters).length > 0) {
      const filterParams = encodeURIComponent(JSON.stringify(activeFilters));
      router.push({ pathname: `/cards/${lower}`, params: { filters: filterParams } } as any);
    } else {
      router.push(`/cards/${lower}` as any);
    }
  };

  if (loading || !isLayoutStable) {
    return (
    ...
    <ThemedView className="flex-1 bg-white dark:bg-gray-900 pt-3">
      ...
      <View className="bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700">
        ...
        { !isEditMode && (
          <ActionButton title="Choose Exercise" onPress={() => setShowPracticeModal(true)} />
        )} 
      {/* Practice Modal */}
      <PracticeModal
        visible={showPracticeModal}
        onClose={() => setShowPracticeModal(false)}
        onSelectMode={handleModeNavigation as any}
      />
    </View>
