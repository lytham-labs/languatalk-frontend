...
interface UpgradeCommunicateCallLimitModalProps {
    isVisible: boolean;
    onClose: () => void;
}

const UpgradeCommunicateCallLimitModal: React.FC<UpgradeCommunicateCallLimitModalProps> = ({
    isVisible,
    onClose,
}) => {
    ...
    const handleUnlimitedPurchaseUpgrade = async (packageId: string) => {
        ...
        if (!activeProductSku) {
            setPurchaseError("No active subscription product SKU found. Cannot upgrade.");
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            setLoading(false);
            setPurchaseError(error instanceof Error ? error.message : 'An unexpected error occurred during purchase.');
        }
    };
                ...
                <View className="flex flex-column justify-between border-1 border-white">
                    ...
                    <View className='flex-1'>
                        ...
                        <ThemedText style={[GlobalFontStyleSheet.textMd, { lineHeight: isTablet ? 38 : 24 }]} className="text-base text-center mb-4">
                            You've used your 30 minutes of daily call time on the Communicate plan. For unlimited practice, please upgrade to Unlimited.
                        </ThemedText>
        ...
        <SlidingModal visible={isVisible} onClose={handleCloseModal}>
            {loading && (
                <View style={[styles.container, { backgroundColor: isDark ? Colors.dark.background : Colors.light.background }]}>
                    <ActivityIndicator className="py-8 flex-1 items-center justify-center" size="large" color={Colors[currentColorScheme]?.tint} />
                </View>
            )}
            {purchaseError && (
...
export default UpgradeCommunicateCallLimitModal; 
