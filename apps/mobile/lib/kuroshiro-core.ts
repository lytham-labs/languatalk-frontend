...
import { PressableSegment } from "@/types/chat";
...
class Kuroshiro {
     ...
     */
    async convert(str: string, options: any, isBold: boolean = false) {
        options = options || {};
        ...
        if (["hiragana", "katakana", "romaji", "all"].indexOf(options.to) === -1) {
        ...
        if (options.mode === "normal" || options.mode === "spaced") {
            switch (options.to) {
                case "katakana":
                    if (options.mode === "normal") {
                        return tokens.map((token: any) => token.reading).join("");
                    }
                    return tokens.map((token: any) => token.reading).join(" ");
                case "romaji":
            ...
            const notations = []; // [basic, basic_type[1=kanji,2=kana,3=others], notation, ruby]
            ...
            let result = "";
            let reactResult:PressableSegment = {
                type: 'ruby',
                text: str,
                isBold,
                segments: []
                ...
                case "katakana":
                    if (options.mode === "okurigana") {
                        ...
                        for (let n1 = 0; n1 < notations.length; n1++) {
                            if (notations[n1][1] !== 1) {
                                reactResult.segments.push({ baseText: notations[n1][0] });
                            }
                            else {
                                reactResult.segments.push({ baseText: notations[n1][0], romajiText: toRawKatakana(notations[n1][2]) });
                            }
                        }
                    }
                ...
                case "romaji":
                    if (options.mode === "okurigana") {
                        ...
                        for (let n3 = 0; n3 < notations.length; n3++) {
                            reactResult.segments.push({ baseText: notations[n3][0], rubyText: toRawRomaji(notations[n3][3], options.romajiSystem) });
                        }
                    }
                    if (options.mode === "furigana") {
                        return reactResult;
                ...
                case "hiragana":
                    if (options.mode === "okurigana") {
                        ...
                        for (let n5 = 0; n5 < notations.length; n5++) {
                            if (notations[n5][1] !== 1) {
                                reactResult.segments.push({ baseText: notations[n5][0]});
                            }
                            else {
                                reactResult.segments.push({ baseText: notations[n5][0], rubyText: notations[n5][2] });
                            }
                        }
                    }
                    if (options.mode === "furigana") {
                ...
                case "all":
                    if (options.mode === "furigana") {
                        for (let n6 = 0; n6 < notations.length; n6++) {
                            reactResult.segments.push({ baseText: notations[n6][0], romaji: toRawRomaji(notations[n6][3], options.romajiSystem), hiragana: notations[n6][1] !== 1 ? null : notations[n6][2] });
                        }
                    }
                    return reactResult;
                default:
